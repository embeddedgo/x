...
func (t Time) abs() uint64 {
	a := t.sec() + internalToAbsolute
	if t.loc != nil && t.loc != &utcLoc {
		_, offset, _, _ := t.loc.lookup(uint64(a))
		a += int64(offset)
	}
	return uint64(a)
}
...
func (t Time) locabs() (name string, offset int, abs uint64) {
	a := t.sec() + internalToAbsolute
	if t.loc != nil && t.loc != &utcLoc {
		name, offset, _, _ = t.loc.lookup(uint64(a))
		a += int64(offset)
	} else {
		name = "UTC"
	}
...
func (t Time) Zone() (name string, offset int) {
	name, offset, _, _ = t.loc.lookup(uint64(t.sec() + internalToAbsolute))
	return
}
...
func (t *Time) UnmarshalBinary(data []byte) error {
	...
	*t = Time{}
	...
	if offset == -1*60 {
		t.setLoc(&utcLoc)
	} else if _, localoff, _, _ := Local.lookup(uint64(t.sec() + internalToAbsolute)); offset == localoff {
		t.setLoc(Local)
	} else {
...
func Date(year int, month Month, day, hour, min, sec, nsec int, loc *Location) Time {
	...
	_, offset, start, end := loc.lookup(abs)
	if offset != 0 {
		switch utc := abs - uint64(int64(offset)); {
		case utc < start:
			_, offset, _, _ = loc.lookup(start - 1)
		case utc >= end:
			_, offset, _, _ = loc.lookup(end)
		}
		abs = uint64(int64(abs) - int64(offset))
	}

	t := Time{uint64(nsec), int64(abs) + absoluteToInternal, nil}
	t.setLoc(loc)
	return t
}
